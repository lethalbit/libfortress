...
#pragma once
#if !defined(libfortress_internal_fd_hh)
#define libfortress_internal_fd_hh
...
#include <libfortress/internal/bits.hh>
#include <libfortress/internal/defs.hh>
#include <libfortress/internal/utility.hh>

// #include <libfortress/internal/mmap.hh>
...
namespace Fortress::Internal {
	...
	public:
		...
		bool read_le(std::uint16_t& val) noexcept {
			if constexpr (Fortress::Internal::is_le()) {
				return read(val);
			} else {
				std::uint16_t _value{};
				const auto res = read(_value);
				val = Fortress::Internal::swap16(_value);
				return res;
			}
		}
		...
		bool write_le(const std::uint16_t val) const noexcept {
			if constexpr (Fortress::Internal::is_le()) {
				...
				return write(Fortress::Internal::swap16(val));
			}
		}
		...
		bool read_le(std::uint32_t& val) noexcept {
			if constexpr (Fortress::Internal::is_le()) {
				...
				std::uint32_t _value{};
				const auto res = read(_value);
				val = Fortress::Internal::swap32(_value);
				return res;
			}
		}
		...
		bool write_le(const std::uint32_t val) const noexcept {
			if constexpr (Fortress::Internal::is_le()) {
				...
				return write(Fortress::Internal::swap32(val));
			}
		}
		...
		bool read_le(std::uint64_t& val) noexcept {
			if constexpr (Fortress::Internal::is_le()) {
				...
				std::uint64_t _value{};
				const auto res = read(_value);
				val = Fortress::Internal::swap64(_value);
				return res;
			}
		}
		...
		bool write_le(const std::uint64_t val) const noexcept {
			if constexpr (Fortress::Internal::is_le()) {
				...
				return write(Fortress::Internal::swap64(val));
			}
		}
		...
		bool read_be(std::uint16_t& val) noexcept {
			if constexpr (Fortress::Internal::is_be()) {
				return read(val);
			} else {
				std::uint16_t _value{};
				const auto res = read(_value);
...
#endif /* libfortress_internal_fd_hh */
